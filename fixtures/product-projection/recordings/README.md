...
run. The smoke flow writes one only when both `PRODUCT_LINEAGE_GOLDEN_OUTPUT`
and the exact Git commit `PRODUCT_LINEAGE_SOURCE_REVISION` are set; a missing or
non-SHA revision fails closed after the provider run. It never inserts rows or
synthesizes a sample. A recording is an atomic directory containing independent `api/*.json`
