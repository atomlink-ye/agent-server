...
if (process.env.WEB_BOOTSTRAP_EMPTY_PRODUCT !== '0') {
  throw new Error(
    'WEB_BOOTSTRAP_EMPTY_PRODUCT=0 is required for the ProductSession browser canary.',
  );
}
const parsedBaseUrl = new URL(baseUrlEnv);
if (!parsedBaseUrl.hostname.endsWith('.localhost')) {
  throw new Error(
    'WEB_E2E_BASE_URL must use a trustworthy .localhost hostname for the ProductSession browser canary.',
  );
}
...
describe('web ProductSession live/replay E2E', () => {
  afterEach(async () => {
    await browser?.close();
    browser = undefined;
  });
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        const browserHostname = parsedBaseUrl.hostname;
