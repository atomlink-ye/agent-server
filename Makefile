.PHONY: setup dev dev-api check-fast web-bootstrap web-dev web-build web-check-types web-e2e-smoke test-web build check test test-unit test-integration test-real-pg test-contract e2e-smoke paseo-smoke eval-smoke ci managed-environment-smoke clean provider-toolchain-acceptance provider-toolchain-status buildkit-cache-acceptance \
	...
	test-contract-native test-web-native e2e-smoke-native paseo-smoke-native eval-smoke-native ci-native clean-native self-learning-team-phase2-smoke self-learning-team-phase3-smoke agent-teams-v2-smoke product-work-projection-smoke claude-provider-smoke mixed-team-journey provider-smoke
...
web-e2e-smoke:
	...
	wait_for_url() { \
		url="$$1"; \
		deadline=$$(expr $$(date +%s) + $$(expr "$$PASEO_DAEMON_STARTUP_TIMEOUT_MS" / 1000) + 60); \
...
internal-dev:
	@. ./scripts/dev/source-real-provider-defaults; pnpm dev
