...
export class PostgresTeamMessageRepository implements TeamMessageRepository {
  ...
  public async create(message: TeamMessage): Promise<TeamMessage> {
    if (message.kind === 'direct')
      throw new Error('Direct Team messages require the fenced send operation.');
    const result = await this.database.query<MessageRow>(
      `INSERT INTO team_messages
       ...
       RETURNING *`,
      [message.id,message.teamRunId,message.tenantId,message.workspaceId,message.principalType,message.principalId,
        message.senderMemberRunId,message.recipientMemberRunId,message.workItemId,message.attemptId,message.aboutWorkItemId,
        message.replyToMessageId,message.kind,message.dedupKey,message.body,message.priority,message.requiresAck,
        message.sourceTaskId,message.sourceRunId,message.createdAt],
    );
    ...
    if (!existing.rows?.[0]) throw new Error('Team message could not be persisted.');
    return mapMessage(existing.rows[0]);
  }

  public async listQueuedWakeRoots() {
    const result = await this.database.query<{
      root_task_id: string; tenant_id: string; workspace_id: string; principal_type: string; principal_id: string;
    ...
    return (result.rows ?? []).map((row) => ({
      ...
      owner: { tenantId: row.tenant_id, workspaceId: row.workspace_id, principalType: row.principal_type, principalId: row.principal_id },
    }));
  }

  public async listQueuedForMember(teamRunId: string, memberId: string, owner: OwnerScope) {
    const result = await this.database.query<MessageRow>(
  ...
  public async bindToTask(input: { messageIds: readonly string[]; taskId: string; owner: OwnerScope }) {
  ...
  public async claimDirectForTask(input: {
    ...
    const claimed = await this.claimDirectBatchForTask({
      messageIds: [input.messageId], taskId: input.taskId, teamRunId: input.teamRunId,
      recipientMemberRunId: input.recipientMemberRunId, owner: input.owner,
    });
  ...
  public async claimDirectBatchForTask(input: {
    messageIds: readonly string[]; taskId: string; teamRunId: string; recipientMemberRunId: string; owner: OwnerScope;
    ...
    const result = await this.database.query<MessageRow>(
      `UPDATE team_messages m SET status='consumed',consumed_by_task_id=$2,consumed_at=now()
        ...
        RETURNING m.*`,
      [input.messageIds,input.taskId,input.teamRunId,input.recipientMemberRunId,...ownerValues(input.owner)],
    );
    ...
    try {
      ...
      if (input.replyToMessageId) {
        const parent = await client.query<MessageRow>(
          ...
          [input.replyToMessageId,input.teamRunId,...ownerValues(input.owner)],
        );
      ...
      const result = await client.query<MessageRow>(
        ...
        [randomUUID(),input.teamRunId,...ownerValues(input.owner),input.senderMemberRunId,input.recipientMemberRunId,
          input.aboutWorkItemId ?? null,input.replyToMessageId ?? null,input.dedupKey,input.body,input.priority ?? 'normal',
          input.requiresAck ?? false,input.sourceTaskId,input.sourceRunId],
      );
      ...
      if (!row) {
        ...
        row=replay.rows?.[0];
        if (!row || row.kind!=='direct' || row.sender_member_run_id!==input.senderMemberRunId ||
            row.recipient_member_run_id!==input.recipientMemberRunId || row.body!==input.body)
  ...
  public async acknowledgeDirect(input: {
    messageId:string; recipientMemberRunId:string; sourceTaskId:string; sourceRunId:string; owner:OwnerScope;
  }): Promise<TeamMessage> {
    const source=await this.database.query(
      ...
      [input.sourceTaskId,input.sourceRunId,input.recipientMemberRunId,...ownerValues(input.owner)],
    );
    if (!source.rows?.[0]) throw new TeamExecutionError('stale_state');
    const result=await this.database.query<MessageRow>(
...
function ownerValues(owner:OwnerScope):readonly string[]{return [owner.tenantId,owner.workspaceId,owner.principalType,owner.principalId];}
function ownerSql(alias:string,start:number):string{const p=alias?`${alias}.`:'';return `${p}tenant_id=$${start} AND ${p}workspace_id=$${start+1} AND ${p}principal_type=$${start+2} AND ${p}principal_id=$${start+3}`;}
function iso(value:string|Date|null):string|null{return value===null?null:value instanceof Date?value.toISOString():value;}
function mapMessage(row:MessageRow):TeamMessage{return Object.freeze({
  id:row.id,teamRunId:row.team_run_id,tenantId:row.tenant_id,workspaceId:row.workspace_id,principalType:row.principal_type,principalId:row.principal_id,
  sequence:Number(row.sequence),senderMemberRunId:row.sender_member_run_id,recipientMemberRunId:row.recipient_member_run_id,workItemId:row.work_item_id,
  attemptId:row.attempt_id,aboutWorkItemId:row.about_work_item_id,replyToMessageId:row.reply_to_message_id,kind:row.kind,dedupKey:row.dedup_key,body:row.body,
  priority:row.priority??'normal',requiresAck:Boolean(row.requires_ack),status:row.status,consumedByTaskId:row.consumed_by_task_id,
  sourceTaskId:row.source_task_id,sourceRunId:row.source_run_id,createdAt:iso(row.created_at)!,consumedAt:iso(row.consumed_at),acknowledgedAt:iso(row.acknowledged_at),cancelledAt:iso(row.cancelled_at),
});}
