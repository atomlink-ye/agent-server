...
async function main() {
  const [{ API }, { createVirtualFileSystem }, ast] = await Promise.all([
    import('typescript/unstable/sync'),
    import('typescript/unstable/fs'),
    import('typescript/unstable/ast'),
  ]);
  if (typeof API !== 'function') throw new Error('typescript_api_unavailable');
  if (typeof createVirtualFileSystem !== 'function')
    throw new Error('typescript_virtual_fs_unavailable');
  if (typeof ast.isIdentifier !== 'function')
    throw new Error('typescript_ast_identifier_api_unavailable');
