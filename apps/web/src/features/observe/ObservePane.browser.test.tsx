...
it('scrolls the real Observe list to its final traced Run on desktop', async () => {
  listWorks.mockResolvedValue({
    works: Array.from({ length: 48 }, (_, index) => ({
      ...WORK_ITEM,
      id: `work-${index}`,
      title: index === 47 ? 'Final real traced Run' : `Traced Work ${index}`,
      latest_run_summary: {
        ...WORK_ITEM.latest_run_summary,
        id: `run-${index}`,
      },
    })),
    next_cursor: null,
  ...
  loadCoworkerProfile.mockResolvedValue({
    workCatalog: [{ definitionId: 'definition-1' }],
  });
  loadSessionTranscripts.mockResolvedValue({
    work_id: 'work-1',
    work_run_id: 'run-1',
  ...
  try {
    ...
    const final = [...region.querySelectorAll('a')].find((item) =>
      item.textContent?.includes('Final real traced Run'),
    )!;
