...
**Coworker Work Catalog baseline:** an enabled binding identifies one Coworker AgentDefinition, one WorkDefinition, and one exact published WorkDefinitionVersion from the same owner/lineage. Definitions are independently discoverable in the Work catalog, including when unbound; the definition-first availability API can bind the same Definition to multiple Coworkers. New Coworkers start with an empty catalog and are guided to choose a Definition explicitly. The catalog tells Chat what formal work the Coworker can start; WorkerVersion remains the execution authority.
...
**Coworker workspace baseline:** Chat-turn-authorized [workspace MCP tools](contracts/runtime-contract.md#coworker-workspace-tools) let a Coworker list, read, and write its own logical files. The product authoring form includes these tools for new Coworkers. Files use canonical ContextFS entries and immutable snapshots and appear in the existing Files Agent scope. No files are seeded at creation; content comes from actual tool calls.
...
New Boards include Todo, Doing, and Done columns with matching declared kinds. Column creation and editing can set or clear a kind independently of the title; these operations do not reclassify other columns. Claiming advances a card from a declared Todo column to a declared Doing column through the backend; the browser refreshes that placement without guessing from column titles or moving a claimed card backwards. See the [persistence boundary](contracts/work-organization-api.md#persistence-and-recovery-boundary) for the historical column-kind migration.
...
**Work-first Web baseline:** the Vite Coworker/Work shell is the canonical browser product. Work renders inside the same shell as Conversations and Agents, opens Work Detail with Overview / Runs / Artifacts / Definition, supports explicit Start Run and Work Definition authoring, and reads the exact Product DefinitionVersion used by the selected current or historical Run. The same-origin Hono BFF keeps the Agent Server bearer server-side. The Work directory shows record state and exact Run counts. The Runs index traverses existing paginated reads for chronological ordinals; selected Run results and execution records appear in a separate Run surface on the existing Work route. Canonical browser tests run in deterministic CI.
