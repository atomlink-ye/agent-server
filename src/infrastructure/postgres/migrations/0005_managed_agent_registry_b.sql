...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'agent_definitions_managed_shape_check') THEN
    ALTER TABLE agent_definitions ADD CONSTRAINT agent_definitions_managed_shape_check CHECK ((
      (managed_discriminator IS NULL AND normalized_name IS NULL)
      OR (managed_discriminator = 'managed_agent_v1' AND normalized_name IS NOT NULL AND length(btrim(normalized_name)) > 0 AND octet_length(normalized_name) <= 255)
    ) IS TRUE);
  END IF;
END
$$;
...
CREATE UNIQUE INDEX IF NOT EXISTS agent_definitions_owner_identity_uq
  ON agent_definitions (id, tenant_id, principal_type, principal_id);
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'agent_versions_managed_shape_check') THEN
    ALTER TABLE agent_versions ADD CONSTRAINT agent_versions_managed_shape_check CHECK ((
...
CREATE UNIQUE INDEX IF NOT EXISTS agent_versions_owner_identity_uq
  ON agent_versions (id, definition_id, tenant_id, principal_type, principal_id);

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'agent_versions_managed_definition_fk') THEN
    ALTER TABLE agent_versions ADD CONSTRAINT agent_versions_managed_definition_fk
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'agent_registry_idempotency_pkey') THEN
    ALTER TABLE agent_registry_idempotency ADD CONSTRAINT agent_registry_idempotency_pkey
      PRIMARY KEY (operation, tenant_id, principal_type, principal_id, idempotency_key);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'agent_registry_idempotency_operation_check') THEN
    ALTER TABLE agent_registry_idempotency ADD CONSTRAINT agent_registry_idempotency_operation_check
      CHECK (operation IN ('import', 'publish'));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'agent_registry_idempotency_key_check') THEN
    ALTER TABLE agent_registry_idempotency ADD CONSTRAINT agent_registry_idempotency_key_check
      CHECK (length(btrim(idempotency_key)) BETWEEN 1 AND 255);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'agent_registry_idempotency_fingerprint_check') THEN
    ALTER TABLE agent_registry_idempotency ADD CONSTRAINT agent_registry_idempotency_fingerprint_check
      CHECK (length(btrim(request_fingerprint)) BETWEEN 1 AND 512);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'agent_registry_idempotency_owner_check') THEN
    ALTER TABLE agent_registry_idempotency ADD CONSTRAINT agent_registry_idempotency_owner_check
      CHECK (length(btrim(tenant_id)) > 0 AND length(btrim(principal_type)) > 0 AND length(btrim(principal_id)) > 0);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'agent_registry_idempotency_timestamp_check') THEN
    ALTER TABLE agent_registry_idempotency ADD CONSTRAINT agent_registry_idempotency_timestamp_check
      CHECK (updated_at >= created_at);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'agent_registry_idempotency_result_check') THEN
    ALTER TABLE agent_registry_idempotency ADD CONSTRAINT agent_registry_idempotency_result_check
      CHECK (((version_id IS NULL) = (definition_id IS NULL)) IS TRUE);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'agent_registry_idempotency_owner_definition_fk') THEN
    ALTER TABLE agent_registry_idempotency ADD CONSTRAINT agent_registry_idempotency_owner_definition_fk
      FOREIGN KEY (definition_id, tenant_id, principal_type, principal_id)
      REFERENCES agent_definitions (id, tenant_id, principal_type, principal_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'agent_registry_idempotency_owner_version_fk') THEN
    ALTER TABLE agent_registry_idempotency ADD CONSTRAINT agent_registry_idempotency_owner_version_fk
      FOREIGN KEY (version_id, definition_id, tenant_id, principal_type, principal_id)
      REFERENCES agent_versions (id, definition_id, tenant_id, principal_type, principal_id);
  END IF;
END
$$;
