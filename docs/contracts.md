...
- [Browser runtime capability projection](contracts/runtime-capabilities-api.md) documents the closed, read-only Work-admission capability snapshot and its four-state UI semantics.
...
- [Runtime contract](contracts/runtime-contract.md) defines provider lifecycle, durable runtime-session readiness, and immutable runtime-tool ownership.
...
The runtime event ABI uses bounded, sanitized `output.payload` projections for
complete `assistant_text` snapshots, cumulative reasoning disclosures, typed
Tool previews, direct-child assistant/reasoning rows, normalized usage, and
read-only permission activity. Run Events are append-only. The Web reducer
replaces snapshots by sequence, preserves first-seen activity order, and uses
the same disclosure projection for live SSE and paged replay; raw, unknown, and
compatibility-final output is ignored. A persisted terminal Run Event closes
