...
const exitCodes = { download_failed: 20, checksum_mismatch: 21, install_failed: 22 };
...
function targetKey() {
  if (process.platform !== 'linux') throw new Error('provider toolchain supports Linux only');
...
function hasExactVersionToken(value, expectedVersion) {
  return value
    .split(/[\/_-]+/u)
    .some((token) => token === expectedVersion || token === `v${expectedVersion}`);
}
...
async function downloadArtifact(provider, artifact, destination) {
  const fileOverride = process.env[envKey('PROVIDER_ARTIFACT', provider) + '_FILE'];
  if (fileOverride) {
    await cp(resolve(fileOverride), destination);
  } else {
    const base = process.env.PROVIDER_ARTIFACT_BASE_URL?.replace(/\/$/u, '');
    const url = base ? `${base}/${provider}/${artifact.url.split('/').at(-1)}` : artifact.url;
    if (!/^https:\/\//u.test(url) || /latest/iu.test(url))
      fail('CHECKSUM_MISMATCH', `artifact URL is not immutable: ${url}`);
    if (!base && !hasExactVersionToken(new URL(artifact.url).pathname, manifest.providers[provider].version))
      fail('CHECKSUM_MISMATCH', `artifact URL does not contain exact provider version: ${provider}`);
    let response;
    try {
      ...
      const wrapped = new Error(`provider artifact download failed: ${provider}`);
      wrapped.code = 'DOWNLOAD_FAILED';
      wrapped.cause = error;
      throw wrapped;
    }
  }
  const expected = process.env[envKey('PROVIDER_ARTIFACT', provider) + '_SHA256'] ?? artifact.sha256;
  if (!expected || !/^[0-9a-f]{64}$/u.test(expected))
    fail('CHECKSUM_MISMATCH', `provider artifact checksum TODO/missing: ${provider}`);
  const actual = await digestFile(destination);
  if (actual !== expected.toLowerCase())
    fail('CHECKSUM_MISMATCH', `provider artifact checksum mismatch: ${provider}`);
}

async function findExecutable(root, provider) {
  const candidates = [provider, provider === 'opencode' ? 'opencode' : null,
    provider === 'codex' ? `codex-${process.arch === 'x64' ? 'x86_64' : 'aarch64'}-unknown-linux-musl` : null].filter(Boolean);
  const queue = [root];
  while (queue.length) {
...
async function installProvider(provider, artifact, releaseRoot) {
  ...
  let binary = null;
  const isExecutable = (await readFile(archive)).subarray(0, 4).toString('hex') === '7f454c46';
  if (isExecutable) binary = archive;
  else {
    const result = spawnSync('tar', ['-xzf', archive, '-C', extract], { encoding: 'utf8' });
    if (result.status !== 0) fail('CHECKSUM_MISMATCH', `provider archive could not be extracted: ${provider}`);
    binary = await findExecutable(extract, provider);
  }
  if (!binary) fail('CHECKSUM_MISMATCH', `provider executable missing from artifact: ${provider}`);
  const target = join(releaseRoot, 'bin', provider);
  await mkdir(dirname(target), { recursive: true });
  await cp(binary, target);
  await chmod(target, 0o755);
  const expectedBinary = process.env[envKey('PROVIDER_BINARY', provider) + '_SHA256'] ?? artifact.binarySha256;
  if (!expectedBinary || !/^[0-9a-f]{64}$/u.test(expectedBinary))
    fail('CHECKSUM_MISMATCH', `installed binary checksum TODO/missing: ${provider}`);
  const actualBinary = await digestFile(target);
  if (actualBinary !== expectedBinary.toLowerCase())
    fail('CHECKSUM_MISMATCH', `installed binary checksum mismatch: ${provider}`);
  const version = spawnSync(target, ['--version'], { encoding: 'utf8', timeout: 120_000 });
  ...
  if (version.status !== 0 || !versionTokens.includes(expectedVersion))
    fail('CHECKSUM_MISMATCH', `provider version mismatch: ${provider} expected ${expectedVersion}`);
  await rm(temp, { recursive: true, force: true });
  return { path: `bin/${provider}`, sha256: actualBinary, version: output.trim().split(/\s+/u).slice(0, 4).join(' ') };
}
...
async function installPaseo(staging) {
  ...
  await Promise.all([
    ...
    cp(join(toolchainRoot, 'pnpm-workspace.yaml'), join(paseo, 'pnpm-workspace.yaml')),
    cp(join(toolchainRoot, 'patches'), join(paseo, 'patches'), { recursive: true }),
  ]);
  const result = spawnSync('pnpm', ['install', '--dir', paseo, '--frozen-lockfile'], {
    encoding: 'utf8',
    stdio: 'inherit',
  });
  if (result.status !== 0) fail('DOWNLOAD_FAILED', 'Paseo toolchain frozen install failed');
  const paseoBin = join(paseo, 'node_modules', '.bin', 'paseo');
  try { await access(paseoBin, constants.X_OK); } catch { fail('CHECKSUM_MISMATCH', 'paseo binary missing after frozen install'); }
...
async function validateRelease(releaseRoot, { requireReady = true } = {}) {
  if (requireReady) {
    ...
    if (ready.trim() !== manifestDigest) fail('CHECKSUM_MISMATCH', 'provider release ready marker mismatch');
  }
  ...
  if (
    ...
    fail('CHECKSUM_MISMATCH', 'provider release toolchain digest or target mismatch');
  ...
  for (const [provider, item] of Object.entries(record.providers)) {
    ...
    const expectedBinary = manifest.providers[provider].artifacts[targetKey()].binarySha256;
    if ((await digestFile(binary)) !== expectedBinary)
      fail('CHECKSUM_MISMATCH', `provider binary checksum mismatch: ${provider}`);
    try { await access(binary, constants.X_OK); } catch { fail('CHECKSUM_MISMATCH', `provider binary not executable: ${provider}`); }
  }
  const paseoBinary = join(releaseRoot, record.paseo?.path ?? '');
  try { await access(paseoBinary, constants.X_OK); } catch { fail('CHECKSUM_MISMATCH', 'paseo binary missing from release'); }
  const paseoVersion = spawnSync(paseoBinary, ['--version'], { encoding: 'utf8', timeout: 120_000 });
  ...
  if (
    ...
    fail('CHECKSUM_MISMATCH', `paseo version mismatch: expected ${manifest.toolchain.paseo.version}`);
  const patch = manifest.toolchain.serverPatch;
  const patchPath = join(releaseRoot, 'paseo-toolchain', 'patches', patch.path.split('/').at(-1));
  if ((await digestFile(patchPath)) !== patch.sha256) fail('CHECKSUM_MISMATCH', 'Paseo server patch checksum mismatch');
  const serverDir = join(releaseRoot, 'paseo-toolchain', 'node_modules', '.pnpm');
  const serverFiles = [];
  const walk = async (root) => {
    for (const entry of await readdir(root, { withFileTypes: true })) {
      const path = join(root, entry.name);
      if (entry.isDirectory()) await walk(path);
      else if (entry.name === 'server-manager.js' || entry.name === 'opencode-agent.js') serverFiles.push(path);
    }
  };
  try { await walk(serverDir); } catch { fail('CHECKSUM_MISMATCH', 'Paseo server output missing'); }
  for (const marker of patch.markers) {
    let found = false;
    for (const path of serverFiles) if ((await readFile(path, 'utf8')).includes(marker)) found = true;
    if (!found) fail('CHECKSUM_MISMATCH', `Paseo patch marker missing: ${marker}`);
  }
  return record;
}
...
async function lockedInit() {
  ...
  if (await access(join(existing, '.ready')).then(() => true).catch(() => false)) {
    try {
      await validateRelease(existing);
      await activateRelease(existing);
      await setStatus('ready', { release: `releases/${manifestDigest}`, target: targetKey(), reused: true });
      return;
    } catch {
  ...
  for (const entry of await readdir(volumeRoot, { withFileTypes: true }))
    if (entry.name.startsWith('staging-')) await rm(join(volumeRoot, entry.name), { recursive: true, force: true });
  const staging = join(volumeRoot, `staging-${process.pid}-${randomUUID()}`);
  await mkdir(staging, { recursive: true });
  await setStatus('installing', { staging: staging.split('/').at(-1), target: targetKey() });
  try {
    ...
    for (const [name, definition] of Object.entries(manifest.providers)) {
      ...
      if (!artifact) fail('CHECKSUM_MISMATCH', `no artifact for ${name}/${targetKey()}`);
      ...
      if (process.env.PROVIDER_TOOLCHAIN_PAUSE_FILE) {
        while (await access(process.env.PROVIDER_TOOLCHAIN_PAUSE_FILE).then(() => true).catch(() => false))
          await new Promise((resolvePause) => setTimeout(resolvePause, 100));
      }
      if (process.env.PROVIDER_TOOLCHAIN_KILL_HOOK && name === 'opencode') {
        spawnSync(process.env.PROVIDER_TOOLCHAIN_KILL_HOOK, { shell: true, stdio: 'inherit' });
      }
    }
    const record = { schemaVersion: 1, manifestDigest, toolchainDigest, target: targetKey(), paseo: { path: paseoPath }, providers };
    ...
    await setStatus('ready', { release: `releases/${manifestDigest}`, target: targetKey() });
  } catch (error) {
    await rm(staging, { recursive: true, force: true });
    const status = failureStatus(error);
    await setStatus(status, { message: error instanceof Error ? error.message : String(error) });
...
async function activateRelease(releaseRoot) {
  ...
  if (resolve(releaseRoot) !== resolve(expected))
    fail('CHECKSUM_MISMATCH', 'refusing to activate an unexpected provider release path');
  const currentTmp = join(volumeRoot, `.current-${process.pid}-${randomUUID()}`);
...
async function validateActivatedRelease() {
  ...
  if (resolve(dirname(current), target) !== resolve(expected))
    fail('CHECKSUM_MISMATCH', 'provider current release link points to an unexpected release');
...
async function runLocked(command) {
  ...
  await mkdir(volumeRoot, { recursive: true });
  const result = spawnSync('flock', ['-x', lockPath, process.execPath, fileURLToPath(import.meta.url), commandName, '--locked'], {
    stdio: 'inherit',
...
try {
  if (commandName === 'stamp') process.stdout.write(`${toolchainDigest}\n`);
    ...
    catch { status = { schemaVersion: 1, manifestDigest, toolchainDigest, status: 'not_installed' }; }
    ...
    await validateActivatedRelease();
    process.stdout.write(`${JSON.stringify({ status: 'ready', manifestDigest, toolchainDigest, release })}\n`);
