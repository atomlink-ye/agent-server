...
-- 0048 stored the long-lived AgentChatRuntime UUID in scope_id because the
-- runtime session did not yet have an epoch dimension. Preserve those rows as
-- epoch 1 when they can be migrated mechanically; newer epoch-fenced rows use
-- the dedicated typed columns below.
...
CREATE UNIQUE INDEX IF NOT EXISTS runtime_sessions_agent_chat_epoch_uq
  ...
  WHERE scope_kind = 'agent_chat'
    AND agent_chat_runtime_id IS NOT NULL
...
ALTER TABLE runtime_sessions
  ...
  ADD CONSTRAINT runtime_sessions_agent_chat_epoch_check CHECK (
    ...
    OR (
      (agent_chat_runtime_id IS NULL AND runtime_epoch IS NULL)
      OR (agent_chat_runtime_id IS NOT NULL AND runtime_epoch IS NOT NULL AND runtime_epoch > 0)
    )
  );
