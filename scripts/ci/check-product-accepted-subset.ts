...
const ACCEPTED_FACTS = {
  ...
  acceptedManifestCanonicalSha256:
...
const SIGNED_LINEAGE = {
  pre_gate_candidate_sha: 'a1d6351c2c8a69a36d52e8b2fb27c654cbe570b7',
  source_acceptance_sha: '5866962675a39791d0a8ac8a2b93e0868dd69345',
  source_acceptance_parent_sha: 'a1d6351c2c8a69a36d52e8b2fb27c654cbe570b7',
  source_accepted_tree_sha: '3ac6f4812130e6694f209fb64650f0baaee6f1c9',
  squash_acceptance_sha: '7731af29cd127dd09583da30ef183c8b5015815c',
  squash_parent_sha: '26406954f548878d8757054bde5db71814fe6a30',
  squash_accepted_tree_sha: '3ac6f4812130e6694f209fb64650f0baaee6f1c9',
  trees_equal: true,
  squash_parent_semantics: 'mainline_integration_parent_not_pre_gate_candidate',
} as const;

function verifyLineage(value: unknown, label: string): Record<string, unknown> {
  if (!isObject(value)) fail(`evidence_mismatch:${label}`);
  equalValue(value, SIGNED_LINEAGE, label);
  return value;
}
...
function verifyFormatProvenance(value: unknown): void {
  ...
  const shaFields = [
    'commit',
    'commit_tree',
    'independently_regenerated_tree',
  ...
  for (const field of shaFields)
    if (
      ...
      !/^[a-f0-9]{40}$/u.test(value[field] as string)
    )
      fail(`evidence_mismatch:continuation.format_provenance.${field}`);
  equalValue(
...
async function verifyAcceptedEvidence(
  ...
  const continuationBytes = await readFile(paths.continuation).catch(() => {
    fail(`evidence_missing:continuation:${paths.continuation}`);
  });
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  try {
    ...
    try {
      manifestBytes = await readFile(options.manifest);
    } catch {
