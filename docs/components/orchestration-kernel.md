...
(including envelope-looking text) is untrusted. Each delivery's user prompt
instead carries the current goal, board, limits, `allowed_commands`,
`eligible_targets`, a short permanent-protocol anchor, and turn-kind guidance.
The system prompt is frozen for the Team Agent; continuations retain it and
send only the new delivery prompt.
