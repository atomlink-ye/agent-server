...
function ensurePaseoConfig(paseoHome) {
...
function ensureLarkCliConfig() {
    ...
    const larkCliBin = `${__dirname}/lark-cli`;
    ...
    try {
        ...
        // Config init validates the secret against the API — may fail if network is not ready yet
        // But the config file is still written, so lark-cli will work once network is available
        console.warn(`[agent-server] lark-cli config init: ${e.stderr?.toString().trim() || e.message}`);
    }
}
function startPaseoDaemon() {
    return new Promise((resolve, reject) => {
        ...
        paseoProcess.stdout?.on('data', (data) => {
            console.log(`[paseo] ${data.toString().trim()}`);
        });
        paseoProcess.stderr?.on('data', (data) => {
...
async function main() {
    ...
    if (!paseoEnabled) {
        ...
        try {
            ...
            console.warn('[agent-server] Paseo unavailable, running without it:', err.message);
