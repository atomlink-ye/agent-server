...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!value || value.startsWith('--') || parsed[name])
...
async function oi38(baseUrl) {
  ...
  const foreignToken =
    process.env.AGENT_SERVER_FOREIGN_TOKEN ?? process.env.OI38_FOREIGN_TOKEN;
  const workId = process.env.PRODUCT_E2E_WORK_ID;
  if (!ownerToken || !foreignToken || !workId)
    fail('oi38_environment_required');
  ...
  if (
    ownerRuns.status !== 200 ||
    !Array.isArray(ownerRuns.body?.work_runs) ||
    ownerRuns.body.work_runs.length < 1
  )
  ...
  if (
    foreign.status !== 404 ||
    missing.status !== 404 ||
    normalize(foreign) !== normalize(missing) ||
  ...
  process.stdout.write(
    JSON.stringify({
      scenario: 'oi38',
      owner_work_run_count: ownerRuns.body.work_runs.length,
...
async function mutationProbe(recording) {
  ...
  try {
    ...
    const checked = spawnSync('sha256sum', ['-c', 'SHA256SUMS'], {
      cwd: copy,
      encoding: 'utf8',
      env: process.env,
    });
...
async function main() {
  ...
  if (scenario && scenario !== 'oi38') {
    ...
    const result = child(
      process.execPath,
      [
        'scripts/record/product-projection-real-run.mjs',
        '--mode',
        'product',
        '--scenario',
        scenario,
  ...
  for (const directory of dirs) {
    ...
    process.stdout.write(
      `${JSON.stringify({ recording: directory, ...result })}\n`,
    );
  }
  await mutationProbe(dirs[0]);
  const foreign = child(process.execPath, [
    'scripts/record/product-projection-real-run.mjs',
    '--mode',
    'product',
    '--scenario',
  ...
  const additive = child(process.execPath, [
    'scripts/smoke/product-consumer-additive-field.mjs',
    '--recording',
    dirs[0],
  ]);
