...
| Agents and Teams                 | Baseline       | **Collaborative and Agentic Team MVE plus compatibility paths:** managed Agent/Team YAML validation, owner-scoped registry/API, immutable published versions, published-only Task admission, durable TeamRun/MemberRun/WorkItem reads, fixed-roster Lead commands, one WorkItem with immutable attempts 1 and 2, persisted rework feedback, TeamMemberRun linkage, reused member sessions, and terminal-ready completion-only grants                                                                                                                                                                                    | Immutable Agent/Team versions and bounded graphs                                                      |
...
| Sessions, Tasks and Runs         | Baseline       | **Phase D minimum implemented:** runtime binding, durable normalized lifecycle events, final assistant Message, replayable SSE, owner-scoped cancellation, plus the Phase C ordered lane; ProductSession Environment pinning and internal RuntimeSession/Cell baseline are implemented; ExecuteRun separates stable Bootstrap from per-Run Turn and uses ProductSession binding for same-Agent continuation; the Web MVE projects safe assistant snapshots, reasoning disclosures, typed Tool previews, direct-child timeline rows, final usage, and read-only permission activity through the same Run Events/SSE lane | Runtime Session V2 create/resume/status, incremental deltas, retry, receipts, recovery                |
| Runtime, Tools and Credentials   | Baseline       | Paseo/OpenCode adapter with native Bootstrap/per-turn separation, per-ProductSession Runtime Cell baseline, same-Agent continuation, zero-key model selection, built-in `agent-server/memory-api` Skill loading, complete assistant snapshots, sanitized reasoning disclosures, typed Tool previews, direct-child timeline rows, final usage, and read-only permission projection                                                                                                                                                                                                                                       | Production isolation, placement, tool gateway, credential broker, approvals, Agent Memory HTTP client |
| Artifacts and Evidence           | Planned        | Result text only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Immutable Artifact versions, evidence, source and child lineage                                       |
| Channels, API and Console        | Baseline       | Authenticated Run + Task + Memory Store/Memory HTTP routes, cursor event replay, terminal-closing SSE, Task cancellation, fixed Lark thread binding, built-in Skill evidence, and a separate Next.js same-origin Web BFF with fresh-session assistant-text streaming plus the Project-first Agentic Team Chat MVE                                                                                                                                                                                                                                                                                                       | Web console, canonical identities, broader Lark adapter                                               |
...
The Collaborative Team MVE uses the registry routes and `/api/v1/tasks:invoke` to create a durable TeamRun with one lead and a persisted roster. The Agentic Team MVE keeps that fixed roster, but lets the Lead issue bounded dynamic commands. Its observed path creates one WorkItem, preserves immutable attempts 1 and 2, persists rework feedback, links Lead/member Tasks to TeamMemberRuns, reuses internal member sessions, and grants only `team_completion_request` once all WorkItems are accepted and no attempt is active. TeamRun and member reads remain owner-scoped. `sequential-mvp-v1` remains the existing compatibility subset. The opt-in `dag-mve-v1` path is observed to materialize two parallel leaf child Tasks/Runs, wait at a root Run in `waiting_children`, durably join both successes, then materialize a synthesizer child and complete the root. The fixed self-learning Team is exposed through the Project-first same-origin Web BFF; the former Project Lab/Web routes are removed. The managed Agent package contract does not claim arbitrary model execution or that referenced tools and skills are available merely because their references parse.
...
The Web Chat + Paseo rich-events MVE adds one narrow internal browser path. A
separate Next.js service owns the same-origin BFF, keeps the Agent Server bearer
server-side, and exposes only an HttpOnly `product_session_id` to the browser.
Native EventSource consumes the BFF SSE. The runtime emits complete
`assistant_text` snapshots plus sanitized reasoning disclosures, typed Tool
detail previews, direct-child assistant/reasoning rows, `usage`, and
`permission` projections. The reducer replaces transient assistant text by
sequence and renders collapsible root/child activity for both live SSE and
replay, then the durable Assistant Message replaces the transient view after
terminal convergence. Provider payloads, credentials, provider IDs, unsafe
paths, and unbounded detail remain excluded.

The Agentic Team Chat MVE adds a fixed local/single-operator Project-first
observation surface. Its same-origin strict BFF launches the configured Team,
projects one safe owner-checked TeamRun, exposes three read-only Agent Sessions,
replays historical Run events, and restores the selected Project/session from
URL-backed state. The BFF keeps the server bearer server-side; it exposes no
RuntimeSession IDs, prompts, raw events, or raw upstream errors. Deployment
beyond the trusted local operator requires a new authentication Human Gate.
ProductSession Chat remains unchanged.
...
- Full Runtime Session V2 create/resume/status APIs, incremental provider deltas, retries/receipts, and production recovery;
- Broader Web console behavior, cancel UI, old-session restart recovery, multiple Team Projects, pagination, and production Web identity/security hardening;
