...
function assertExactCollection(label, left, right) {
  ...
  if (
    [...leftCounts.values()].some((count) => count !== 1) ||
    [...rightCounts.values()].some((count) => count !== 1)
  )
...
function flattenManifestEntries(value) {
  if (Array.isArray(value))
    return value
      .filter(
        (entry) => isObject(entry) && typeof entry.json_pointer === 'string',
      )
  ...
  if (isObject(candidates))
    return Object.entries(candidates).map(([key, entry]) => ({ key, entry }));
  if (
...
async function loadSchemas() {
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      throw new Error(
        `schema_loader_failed:${error instanceof Error ? error.message : 'unknown'}`,
      );
    }
  }
...
function schemaForFile(file, schemas) {
  if (file === 'product-work-run.json')
    return schemas.find(
      ([name]) => name === 'ProductWorkRunResponseSchema',
    )?.[1];
  if (file === 'product-trace.json')
    return schemas.find(
      ([name]) => name === 'ProductRunTraceResponseSchema',
    )?.[1];
  const stem = file
    .replace(/\.json$/u, '')
    .toLowerCase()
...
async function verifyHashes(sample, manifest) {
  ...
  for (const line of lines) {
    ...
    if (!(await regularFile(join(sample, path))))
      throw new Error(`recording_file_not_regular:${path}`);
    if (path === 'SHA256SUMS' || path.includes('..'))
      throw new Error('sha256sums_path_invalid');
    const content = await readFile(join(sample, path)).catch(() => {
      throw new Error(`recording_file_missing:${path}`);
    });
    if (sha256(content) !== match.groups.hash)
      throw new Error(`recording_hash_mismatch:${path}`);
  }
  if (
    manifest.api_files?.some(
      (file) => !lines.some((line) => line.endsWith(`  ${file}`)),
    )
  )
    throw new Error('manifest_api_file_not_hashed');
  if (
    manifest.db_files?.some(
      (file) => !lines.some((line) => line.endsWith(`  ${file}`)),
    )
  )
    throw new Error('manifest_db_file_not_hashed');
  if (!isObject(manifest.file_sha256))
    throw new Error('manifest_file_sha256_missing');
  const sumPaths = new Set(
    lines.map((line) => line.replace(/^[a-f0-9]{64}  /u, '')),
  );
  if (sumPaths.size !== lines.length)
    throw new Error('sha256sums_duplicate_path');
  const requiredSumPaths = new Set([
    'manifest.json',
    ...(manifest.api_files ?? []),
    ...(manifest.db_files ?? []),
  ]);
  if (
    sumPaths.size !== requiredSumPaths.size ||
    [...requiredSumPaths].some((path) => !sumPaths.has(path))
  )
  ...
  for (const file of [...expectedFiles].filter(
    ...
    if (!listedFiles.has(file))
      throw new Error(`manifest_file_sha256_missing:${file}`);
  if (
    listedFiles.size !== expectedFiles.size - 2 ||
    [...listedFiles].some((file) => !expectedFiles.has(file))
  )
  ...
  for (const [path, expected] of Object.entries(manifest.file_sha256)) {
    ...
    if (sha256(content) !== expected)
      throw new Error(`manifest_file_hash_mismatch:${path}`);
  }
}
async function main() {
  ...
  try {
    ...
    if (
      !isObject(manifest) ||
      manifest.format_version !== 'product-lineage-golden-v1'
    )
    ...
    if (
      !isObject(manifest.ids) ||
      !manifest.ids.work_id ||
      !manifest.ids.work_run_id ||
      !manifest.ids.root_task_id ||
      !manifest.ids.team_run_id
    )
      throw new Error('manifest_ids_invalid');
    if (!isObject(manifest.correlation))
      throw new Error('manifest_correlation_invalid');
    if (!Array.isArray(manifest.api_files) || !Array.isArray(manifest.db_files))
      throw new Error('manifest_files_invalid');
    const requiredApiFiles = [
      'api/product-work-run.json',
      'api/product-trace.json',
    ];
    if (
      manifest.api_files.length !== requiredApiFiles.length ||
    ...
    if (
      manifest.db_files.length !== requiredDbFiles.size ||
      manifest.db_files.some((file) => !requiredDbFiles.has(file))
    )
    ...
    if (
      top.some(
        (entry) =>
          !['api', 'db', 'manifest.json', 'SHA256SUMS'].includes(entry.name) ||
          ((entry.name === 'api' || entry.name === 'db') &&
            !entry.isDirectory()),
      )
    )
      throw new Error('recording_extra_entry');
    for (const directory of ['api', 'db']) {
      const entries = await readdir(join(sample, directory), {
        withFileTypes: true,
      }).catch(() => {
        throw new Error(`recording_directory_missing:${directory}`);
      });
      const expected = (
        directory === 'api' ? manifest.api_files : manifest.db_files
      ).map((file) => file.slice(directory.length + 1));
      if (
        entries.some(
          (entry) => !entry.isFile() || !expected.includes(entry.name),
        ) ||
    ...
    if (
      !(await regularFile(join(sample, 'manifest.json'))) ||
      !(await regularFile(join(sample, 'SHA256SUMS')))
    )
      throw new Error('recording_manifest_or_sums_not_regular');
    await verifyHashes(sample, manifest);
    const schemas = await loadSchemas();
    if (!schemas.length)
      throw new Error('product_projection_zod_schemas_empty');
    let apiCount = 0;
    const apiRecords = [];
    for (const file of manifest.api_files) {
      if (!/^api\/[^/]+\.json$/u.test(file))
        throw new Error('manifest_api_path_invalid');
      ...
      apiRecords.push({
        ...
        value,
        owner:
          file === 'api/product-work-run.json'
            ? 'work_run_response'
            : 'run_trace_response',
      });
      apiCount += 1;
    ...
    for (const file of manifest.db_files) {
      if (!/^db\/[^/]+\.json$/u.test(file))
        throw new Error('manifest_db_path_invalid');
      const rows = await readJson(join(sample, file));
      if (!Array.isArray(rows)) throw new Error(`db_rows_not_array:${file}`);
      if (
        file === 'db/team_work_item_attempts.json' &&
        rows.some(
          (row) =>
            Object.hasOwn(row, 'feedback') ||
            Object.hasOwn(row, 'result_summary'),
        )
      )
        throw new Error('db_attempt_text_not_sanitized');
      if (
        file === 'db/team_messages.json' &&
        rows.some((row) => Object.hasOwn(row, 'body'))
      )
        throw new Error('db_message_body_not_sanitized');
      if (
        file === 'db/run_events.json' &&
        rows.some((row) => Object.hasOwn(row, 'payload'))
      )
        throw new Error('db_event_payload_not_sanitized');
      dbCount += rows.length;
    }
    if (!isObject(manifest.query_row_counts))
      throw new Error('manifest_query_row_counts_missing');
    const expectedQueryTables = [...requiredDbFiles].map((file) =>
      file.slice(3, -5),
    );
    const queryCountKeys = Object.keys(manifest.query_row_counts);
    if (
      queryCountKeys.length !== expectedQueryTables.length ||
      expectedQueryTables.some(
        (table) => !Object.hasOwn(manifest.query_row_counts, table),
      ) ||
    ...
    for (const table of expectedQueryTables) {
      ...
      if (
        ...
        count < 0 ||
        count !== (await readJson(join(sample, `db/${table}.json`))).length
      )
    ...
    const lineageCandidates = [
      ...
      join(
        root,
        'src/application/product-projection/lineage/product-lineage-manifest.ts',
      ),
    ];
    let lineageModule;
    ...
    const applicable = entries.filter(({ key }) =>
      String(key).includes('.success::'),
    );
    const source = applicable.filter(
      ({ entry }) =>
        entry?.source_kind === 'column' ||
        entry?.source_kind === 'source_ref' ||
        entry?.kind === 'column' ||
        entry?.kind === 'source_ref',
    );
    const derived = applicable.filter(
      ({ entry }) =>
        (entry?.source_kind === 'derivation' && entry?.derivation_id) ||
        (entry?.kind === 'derivation' && entry?.name),
    );
    if (!source.length || !derived.length)
      throw new Error('lineage_manifest_sampling_classes_missing');
    ...
    const relative = (key) =>
      String(key).replace(/^.*::/u, '').replace(/^\//u, '');
    const walkOccurrences = (value, path) => {
      ...
      const visit = (current, index, objects) => {
        ...
        if (edge) {
          const values = Array.isArray(current[edge[1]])
            ? current[edge[1]].filter((item) => item?.kind === edge[2])
            : [];
          return values.flatMap((item) =>
            visit(item, index + 1, [...objects, current]),
          );
        }
        if (array) {
          const values = Array.isArray(current[array[1]])
            ? current[array[1]]
            : [];
          return values.flatMap((item) =>
            visit(item, index + 1, [...objects, current]),
    ...
    const latestObject = (objects, predicate) =>
      [...objects].reverse().find((item) => isObject(item) && predicate(item));
    const sourceRowFor = (table, occurrence, record) => {
      const objects = occurrence.objects;
      const work = latestObject(
        objects,
        (item) =>
          Object.hasOwn(item, 'work_run') && Object.hasOwn(item, 'work'),
      );
      const workRun = latestObject(
        objects,
        (item) =>
          Object.hasOwn(item, 'work_id') && Object.hasOwn(item, 'trigger_kind'),
      );
      const item = latestObject(
        objects,
        (item) =>
          Array.isArray(item.attempts) && Object.hasOwn(item, 'dependency_ids'),
      );
      const attempt = latestObject(
        objects,
        (item) =>
          Object.hasOwn(item, 'attempt_no') &&
          Object.hasOwn(item, 'feedback_capture_status'),
      );
      const actor = latestObject(
        objects,
        (item) =>
          Object.hasOwn(item, 'name') && Object.hasOwn(item, 'source_refs'),
      );
      const message = latestObject(
        objects,
        (item) =>
          Object.hasOwn(item, 'summary_capture_status') &&
          Object.hasOwn(item, 'sender_id'),
      );
      const run = latestObject(
        objects,
        (item) =>
          Object.hasOwn(item, 'result_capture_status') &&
          Object.hasOwn(item, 'source_refs'),
      );
      const event = latestObject(
        objects,
        (item) =>
          Object.hasOwn(item, 'payload_capture_status') &&
          Object.hasOwn(item, 'sequence'),
      );
      const edge = latestObject(
        objects,
        (item) =>
          typeof item.kind === 'string' && Object.hasOwn(item, 'source_refs'),
      );
      const refs = (object) => object?.source_refs ?? {};
      const rows = dbByTable.get(table) ?? [];
      return rows.find((row) => {
        if (table === 'works')
          return row.id === (record.value.work?.id ?? manifest.ids.work_id);
        if (table === 'work_runs')
          return (
            row.id === (record.value.work_run?.id ?? manifest.ids.work_run_id)
          );
        if (table === 'team_work_items')
          return (
            row.id === item?.id &&
            (!row.team_run_id || row.team_run_id === refs(item).team_run_id)
          );
        if (table === 'team_work_item_attempts')
          return (
            row.id === attempt?.id &&
            (!row.team_run_id || row.team_run_id === refs(attempt).team_run_id)
          );
        if (table === 'team_member_runs')
          return (
            row.id === actor?.id &&
            (!row.team_run_id || row.team_run_id === refs(actor).team_run_id)
          );
        if (table === 'team_messages')
          return (
            row.id === (message?.id ?? edge?.message_id) &&
            (!row.team_run_id ||
              row.team_run_id === refs(message ?? edge).team_run_id)
          );
        if (table === 'runs') return row.id === refs(run).run_id;
        if (table === 'run_events')
          return (
            row.run_id === refs(event).run_id &&
            row.sequence === event?.sequence
          );
        if (table === 'team_work_item_dependencies')
          return (
            row.team_run_id === refs(edge).team_run_id &&
            row.work_item_id ===
              (edge?.dependent_work_item_id ?? edge?.work_item_id) &&
            row.depends_on_work_item_id ===
              (edge?.prerequisite_work_item_id ?? edge?.dependency_id)
          );
        if (table === 'tasks') return row.id === refs(edge).task_id;
        return false;
      });
    };
    const dbValues = (table, column) =>
      (dbByTable.get(table) ?? []).map((row) => normalize(row[column]));
    const responseByOwner = (owner) =>
      apiRecords.find((record) => record.owner === owner)?.value;
    for (const record of apiRecords) {
      const prefix = record.owner === 'work_run_response' ? 'work' : 'trace';
      assertExactCollection(
        `${prefix}.work.id`,
        [record.value.work?.id].filter((value) => value !== undefined),
        dbValues('works', 'id'),
      );
      assertExactCollection(
        `${prefix}.work_run.id`,
        [record.value.work_run?.id].filter((value) => value !== undefined),
        dbValues('work_runs', 'id'),
      );
      assertExactCollection(
        `${prefix}.work_items.id`,
        (record.value.work_items ?? []).map((item) => item.id),
        dbValues('team_work_items', 'id'),
      );
      assertExactCollection(
        `${prefix}.attempts.id`,
        (record.value.work_items ?? []).flatMap((item) =>
          item.attempts.map((attempt) => attempt.id),
        ),
        dbValues('team_work_item_attempts', 'id'),
      );
      assertExactCollection(
        `${prefix}.actors.id`,
        (record.value.actors ?? []).map((actor) => actor.id),
        dbValues('team_member_runs', 'id'),
      );
      assertExactCollection(
        `${prefix}.messages.id`,
        (record.value.messages ?? []).map((message) => message.id),
        dbValues('team_messages', 'id'),
      );
    }
    ...
    assertExactCollection(
      'trace.runs.id',
      (trace.runs ?? []).map((run) => run.source_refs?.run_id),
      dbValues('runs', 'id'),
    );
    assertExactCollection(
      'trace.events.run_id_sequence',
      (trace.events ?? []).map((event) =>
        JSON.stringify([event.source_refs?.run_id, event.sequence]),
      ),
      (dbByTable.get('run_events') ?? []).map((row) =>
        JSON.stringify([row.run_id, row.sequence]),
      ),
    );
    const taskById = new Map(
      (dbByTable.get('tasks') ?? []).map((row) => [row.id, row]),
    );
    const edgeTuple = (kind, edge) => {
      if (kind === 'observed_message')
        return [
          edge.source_refs?.team_run_id,
          edge.message_id,
          edge.sequence,
          edge.sender_actor_id,
          edge.recipient_actor_id,
          edge.work_item_id,
          edge.attempt_id,
          edge.source_created_at,
        ];
      if (kind === 'declared_dependency')
        return [
          edge.source_refs?.team_run_id,
          edge.dependent_work_item_id,
          edge.prerequisite_work_item_id,
          edge.source_created_at,
        ];
      if (kind === 'assignment')
        return [
          edge.source_refs?.team_run_id,
          edge.source_refs?.task_id,
          edge.attempt_id,
          edge.work_item_id,
          edge.assignee_actor_id,
          edge.source_created_at,
        ];
      return [
        edge.source_refs?.team_run_id,
    ...
    const dbEdgeTuple = (kind, row) => {
      if (kind === 'observed_message')
        return [
          ...
          row.id,
          row.sequence,
          row.sender_member_run_id,
          row.recipient_member_run_id,
          row.work_item_id,
          row.attempt_id,
          row.created_at,
        ];
      ...
      const reviewer =
        taskById.get(row.requested_by_lead_task_id)?.team_member_run_id ?? null;
      return [
        row.team_run_id,
        row.requested_by_lead_task_id,
        row.id,
        row.work_item_id,
        kind === 'feedback' ? reviewer : row.assignee_member_id,
        row.created_at,
      ];
    };
    for (const kind of [
      ...
      const responseEdges = (trace.edges ?? []).filter(
        (edge) => edge.kind === kind,
      );
      const sourceRows = (
        dbByTable.get(
      ...
      ).filter((row) => kind !== 'feedback' || row.feedback_present === true);
      assertExactCollection(
        `trace.edges.${kind}`,
        responseEdges.map((edge) => JSON.stringify(edgeTuple(kind, edge))),
        sourceRows.map((row) => JSON.stringify(dbEdgeTuple(kind, row))),
      );
    }
    const supportedFormula = (formula) =>
      typeof formula === 'string' &&
      (/^constant\(/u.test(formula) ||
        /capture_to_null/u.test(formula) ||
        /presence_to_redaction_status/u.test(formula) ||
        /capture_status\(redacted_or_absent\)/u.test(formula) ||
        /collect\(depends_on_work_item_id\)/u.test(formula) ||
        /json_extract\(/u.test(formula));
    const sourceApplicable = source.filter(({ key, entry }) => {
      ...
      return (
        ...
        sourceRowFor(
          entry.table ?? String(entry.source ?? '').split('.')[0],
          occurrence,
          responseFor(key),
        )
      );
    });
    const derivedApplicable = derived.filter(({ entry }) =>
      supportedFormula(entry.formula),
    );
    if (!sourceApplicable.length)
      throw new Error('golden_source_occurrences_empty');
    if (!derivedApplicable.length)
      throw new Error('golden_derived_occurrences_empty');
    const formulaFamily = (formula) =>
      /collect\(/u.test(formula)
        ? 'collect'
        : /presence_to_redaction_status|capture_status\(redacted_or_absent\)/u.test(
              formula,
            )
          ? 'presence'
          : /json_extract\(/u.test(formula)
      ...
      if (ensureFamilies) {
        ...
        for (const family of [
          ...
          const candidate = ordered.find(
            (item) => formulaFamily(item.entry.formula) === family,
          );
          if (candidate) chosen.push(candidate);
        }
        return [
          ...chosen,
          ...ordered.filter((item) => !chosen.includes(item)),
        ].slice(0, Math.min(count, list.length));
      }
      return ordered.slice(0, Math.min(count, list.length));
    };
    const sourceSamples = sampleEntries(
      sourceApplicable,
      Math.min(3, sourceApplicable.length),
    );
    let mismatches = 0;
    for (const { key, entry } of sourceSamples) {
      ...
      const row =
        occurrence && record
          ? sourceRowFor(table, occurrence, record)
          : undefined;
      if (
        !row ||
        !column ||
        occurrence.value === undefined ||
        !equal(row[column], occurrence.value)
      )
        mismatches += 1;
    }
    ...
    for (const { key, entry } of derivedApplicable) {
      ...
      for (const occurrence of occurrencesFor(key)) {
        ...
        if (constant)
          ...
          if (row && jsonField)
            expected =
              row[jsonField === 'code' ? 'error_code' : jsonField] ?? null;
        }
    ...
    if (!derivedCandidates.length)
      throw new Error('golden_derived_occurrences_empty');
    const derivedSamples = sampleEntries(
      derivedCandidates,
      Math.min(8, derivedCandidates.length),
      true,
    );
    for (const sample of derivedSamples)
      if (!equal(sample.expected, sample.actual)) mismatches += 1;
    if (!sourceSamples.length || !derivedSamples.length)
      throw new Error('golden_samples_empty');
    if (mismatches) throw new Error(`lineage_sample_mismatches:${mismatches}`);
    process.stdout.write(
      `source_samples=${sourceSamples.length} derived_samples=${derivedSamples.length} mismatches=${mismatches} seed=${seed}\n`,
    );
