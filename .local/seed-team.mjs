...
const teamRunId = (await query('SELECT id FROM team_runs WHERE root_task_id=$1', [rootTaskId]))[0]?.id;
assert(teamRunId, 'team_run_missing');
const workRows = await query(`SELECT w.id AS work_ref,w.subject,w.status AS work_status,w.created_at,w.updated_at,m.name AS assignee_name,m.role AS assignee_role,c.name AS created_by_name,c.role AS created_by_role FROM team_work_items w JOIN team_member_runs m ON m.id=w.owner_member_id JOIN team_member_runs c ON c.id=w.created_by_member_id WHERE w.team_run_id=$1 ORDER BY w.created_at,w.id`, [teamRunId]);
const createReceipts = await query(`SELECT c.source_run_id,c.created_at AS receipt_created_at,c.result_json,t.id AS task_id,m.name AS source_member_name,d.id AS dispatch_id FROM team_command_receipts c JOIN runs r ON r.id=c.source_run_id JOIN tasks t ON t.id=r.task_id JOIN team_member_runs m ON m.id=t.team_member_run_id LEFT JOIN run_dispatches d ON d.run_id=r.id AND d.event_type='run.enqueue' WHERE t.root_task_id=$1 AND c.command_name='team_work_create' ORDER BY c.created_at,c.command_hash`, [rootTaskId]);
const dependencyRows = await query('SELECT work_item_id,depends_on_work_item_id FROM team_work_item_dependencies WHERE team_run_id=$1', [teamRunId]);
...
assert(createReceipts.length === 2 && new Set(createReceipts.map((row) => row.source_run_id)).size === 1 && createReceipts.every((row) => row.source_member_name === 'lead'), 'exactly_two_kickoff_create_receipts_required');
assert(createReceipts.every((row) => row.task_id && row.dispatch_id), 'kickoff_create_receipt_dispatch_missing');
assert(dependencyRows.length === 0, 'independent_work_dependency_rows_present');
const attemptRows = await query(`SELECT a.id AS attempt_ref,a.work_item_id AS work_ref,a.attempt_no,a.execution_task_id,a.status AS attempt_status,a.result_summary,a.feedback,a.created_at,a.updated_at,a.completed_at,m.name AS assignee_name,t.status AS task_status,r.id AS run_id,r.status AS run_status,d.id AS dispatch_id,d.created_at AS dispatch_created_at,min(e.created_at) FILTER (WHERE e.type='started') AS started_at,max(e.created_at) FILTER (WHERE e.type IN ('succeeded','failed','timed_out','cancelled')) AS terminal_at FROM team_work_item_attempts a JOIN team_member_runs m ON m.id=a.assignee_member_id JOIN tasks t ON t.id=a.execution_task_id JOIN runs r ON r.task_id=t.id LEFT JOIN run_dispatches d ON d.run_id=r.id AND d.event_type='run.enqueue' LEFT JOIN run_events e ON e.run_id=r.id WHERE a.team_run_id=$1 GROUP BY a.id,a.work_item_id,a.attempt_no,a.execution_task_id,a.status,a.result_summary,a.feedback,a.created_at,a.updated_at,a.completed_at,m.name,t.status,r.id,r.status,d.id,d.created_at ORDER BY d.id,a.attempt_no,a.id`, [teamRunId]);
assert(attemptRows.every((row) => row.dispatch_id), 'attempt_dispatch_binding_missing');
assert(createReceipts.every((row) => row.result_json?.item_id && row.result_json?.attempt_id), 'kickoff_create_receipt_binding_missing');
const kickoffAttempts = createReceipts.map((row) => attemptRows.find((attempt) => attempt.attempt_ref === row.result_json.attempt_id));
assert(kickoffAttempts.every(Boolean) && new Set(kickoffAttempts.map((attempt) => attempt.attempt_ref)).size === 2, 'kickoff_attempt_binding_invalid');
const kickoffOrder = [...kickoffAttempts].sort((a, b) => Number(a.dispatch_id) - Number(b.dispatch_id));
assert(kickoffOrder[0].assignee_name === 'fixer' && kickoffOrder[1].assignee_name === 'reviewer', 'kickoff_work_order_invalid');
...
assert(reviewerV1?.attempt_status === 'completed' && reviewerV1.result_summary?.includes('REVIEW_REJECT') && reviewerV1.result_summary?.includes(reviewReason), 'reviewer_reject_invalid');
assert(fixerV2?.attempt_status === 'completed' && fixerV2.result_summary?.includes('FIXER_SUBMIT_V2'), 'fixer_v2_invalid');
assert(fixerV1 && reviewerV1 && fixerV2 && Number(fixerV1.dispatch_id) < Number(reviewerV1.dispatch_id), 'kickoff_dispatch_order_invalid');
const leadDispatchRows = await query(`SELECT t.id AS task_id,t.team_sequence,r.id AS run_id,r.status AS run_status,d.id AS dispatch_id,d.created_at AS dispatch_created_at,min(e.created_at) FILTER (WHERE e.type='started') AS started_at,max(e.created_at) FILTER (WHERE e.type IN ('succeeded','failed','timed_out','cancelled')) AS terminal_at FROM tasks t JOIN runs r ON r.task_id=t.id LEFT JOIN run_dispatches d ON d.run_id=r.id AND d.event_type='run.enqueue' LEFT JOIN run_events e ON e.run_id=r.id WHERE t.root_task_id=$1 AND t.team_task_kind='lead_turn' GROUP BY t.id,t.team_sequence,r.id,r.status,d.id,d.created_at ORDER BY d.id`, [rootTaskId]);
assert(leadDispatchRows[0]?.run_id === createReceipts[0].source_run_id, 'kickoff_receipts_not_earliest_lead_dispatch');
const requestChangesReceipt = await query(`SELECT c.source_run_id,c.created_at AS receipt_created_at,c.result_json,t.id AS task_id,d.id AS dispatch_id,d.created_at AS dispatch_created_at,min(e.created_at) FILTER (WHERE e.type='started') AS started_at,max(e.created_at) FILTER (WHERE e.type IN ('succeeded','failed','timed_out','cancelled')) AS terminal_at FROM team_command_receipts c JOIN runs r ON r.id=c.source_run_id JOIN tasks t ON t.id=r.task_id LEFT JOIN run_dispatches d ON d.run_id=r.id AND d.event_type='run.enqueue' LEFT JOIN run_events e ON e.run_id=r.id WHERE t.root_task_id=$1 AND c.command_name='team_work_request_changes' GROUP BY c.source_run_id,c.created_at,c.result_json,t.id,d.id,d.created_at ORDER BY c.created_at DESC LIMIT 1`, [rootTaskId]);
const requestChanges = requestChangesReceipt[0];
assert(requestChanges?.dispatch_id && Number(reviewerV1.dispatch_id) < Number(requestChanges.dispatch_id), 'review_lead_dispatch_order_invalid');
assert(requestChanges.result_json?.attempt_id === fixerV2.attempt_ref, 'request_changes_attempt_binding_invalid');
assert(fixerV1.started_at && reviewerV1.started_at && fixerV1.terminal_at && reviewerV1.terminal_at && new Date(fixerV1.terminal_at) <= new Date(reviewerV1.started_at), 'fixer_v1_terminal_after_reviewer_start');
assert(new Date(reviewerV1.completed_at ?? reviewerV1.terminal_at) <= new Date(requestChanges.receipt_created_at), 'reviewer_completion_after_request_changes');
assert(fixerV2.feedback?.includes(reviewReason), 'fixer_v2_feedback_missing_exact_review_reason');
assert(fixerV2.started_at && new Date(fixerV2.started_at) >= new Date(requestChanges.receipt_created_at), 'fixer_v2_started_before_request_changes');
const reviewerWork = workRows.find((row) => row.assignee_name === 'reviewer');
const fixerWork = workRows.find((row) => row.assignee_name === 'fixer');
const acceptReceipts = await query(`SELECT c.created_at AS receipt_created_at,c.result_json FROM team_command_receipts c JOIN runs r ON r.id=c.source_run_id JOIN tasks t ON t.id=r.task_id WHERE t.root_task_id=$1 AND c.command_name='team_work_accept' ORDER BY c.created_at`, [rootTaskId]);
const reviewerAccept = acceptReceipts.find((row) => row.result_json?.work_item_id === reviewerWork?.work_ref);
const fixerAccept = acceptReceipts.find((row) => row.result_json?.work_item_id === fixerWork?.work_ref);
assert(reviewerAccept && new Date(reviewerAccept.receipt_created_at) >= new Date(reviewerV1.completed_at ?? reviewerV1.terminal_at), 'reviewer_accept_before_completion');
assert(fixerAccept && new Date(fixerAccept.receipt_created_at) >= new Date(fixerV2.completed_at ?? fixerV2.terminal_at), 'fixer_v2_accept_before_completion');
const nonterminalChildren = await query(`SELECT t.id AS task_id,t.status AS task_status,r.status AS run_status FROM tasks t LEFT JOIN runs r ON r.task_id=t.id WHERE t.root_task_id=$1 AND t.id<>$1 AND (t.status NOT IN ('completed','failed','cancelled') OR (r.id IS NOT NULL AND r.status NOT IN ('succeeded','failed','timed_out','cancelled')))` , [rootTaskId]);
assert(nonterminalChildren.length === 0, 'nonterminal_children_present');
const dispatchIntervals = [...attemptRows, ...leadDispatchRows].filter((row) => row.started_at && row.terminal_at).map((row) => ({ dispatch_id: row.dispatch_id, started_at: row.started_at, terminal_at: row.terminal_at })).sort((a, b) => new Date(a.started_at) - new Date(b.started_at) || Number(a.dispatch_id) - Number(b.dispatch_id));
let effectiveConcurrency = 0;
let activeTerminals = [];
for (const interval of dispatchIntervals) {
  const startedAt = new Date(interval.started_at);
  activeTerminals = activeTerminals.filter((terminalAt) => terminalAt > startedAt);
...
const rootTerminalRows = await query(`SELECT t.id AS task_id,t.status AS task_status,r.id AS run_id,r.status AS run_status FROM tasks t JOIN runs r ON r.task_id=t.id WHERE t.id=$1 AND t.root_task_id=$1`, [rootTaskId]);
assert(rootTerminalRows.length === 1 && rootTerminalRows[0].task_status === 'completed' && rootTerminalRows[0].run_status === 'succeeded', 'durable_root_terminal_success_invalid');
await writeFile(`${evidenceRoot}/workflow.json`, JSON.stringify({ root_task_id: rootTaskId, team_status: projection.project.status, root_terminal: rootTerminalRows[0], work: workRows, attempts: attemptRows, create_receipts: createReceipts, dependencies: dependencyRows, request_changes: requestChanges, accepts: acceptReceipts, nonterminal_children: nonterminalChildren, effective_concurrency: effectiveConcurrency, chronological }, null, 2));
...
assert(reviewerV1.terminal_at && requestChanges.started_at && new Date(reviewerV1.terminal_at) <= new Date(requestChanges.started_at), 'reviewer_terminal_after_queued_lead_start');
await writeFile(`${evidenceRoot}/dispatch-order.json`, JSON.stringify({ fixer_v1: fixerV1, reviewer_v1: reviewerV1, queued_review_lead: requestChanges }, null, 2));
...
await writeFile(`${evidenceRoot}/manifest.json`, JSON.stringify({ schema: 'mixed-team-proof-v1', root_task_id: rootTaskId, composition: { provider_used: compositionProviderUsed, model: resolvedModel, models: memberModels, scripted_runtime: scriptedRuntimeDetected, members: ['lead', 'fixer', 'reviewer'] }, ordering_mode: 'observed_dispatch_id_fifo_single_observed_concurrency', effective_concurrency: effectiveConcurrency, platform_sequence_guarantee: false, normal_concurrency_reachable: true, evidence: ['providers.json', 'workflow.json', 'workspace.json', 'dispatch-order.json', 'python-artifact.json', 'mixed_team_rework.v1.py', 'mixed_team_rework.py'] }, null, 2));
await db.end();
