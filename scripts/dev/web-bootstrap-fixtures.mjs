...
function mixedTeamInstructions(name) {
  ...
  return `Act directly as the assigned reviewer using canonical Team tools plus the available workspace terminal. Do not create or mutate another Work and do not use provider subagents. Starting from the current working directory, walk parent directories until package.json has name @atomlink-ye/agent-server and compose.yaml is present; never use version-control commands or print an absolute path. Then read and run repo-root relative mixed_team_rework.v1.py (the preserved first attempt) with --text ''. Genuinely verify the declared tokens[0] defect produces an IndexError, and use board_submit to submit a completed result containing marker REVIEW_REJECT and exact blocking reason: "empty input is mishandled: the utility indexes the first token instead of returning an empty result." Include the relative path and safe run observation. Do not edit either file, do not accept Work, never send messages, and stop after one submit.`;
}
