...
| Agents and Teams                 | Baseline       | **Agent Teams v2:** managed Agent/Team YAML validation, owner-scoped registry/API, immutable published versions, published-only Task admission, TeamDriver activation, durable TeamRun/MemberRun/Work/TeamMessage reads, fixed-roster Lead commands, bounded work attempts, revision-fenced commands, and addressed continuations                                                                                                                                                                                                                                                                                       | Dynamic rosters, nested Teams, and generalized graphs                                                 |
...
Agent Teams v2 uses the registry routes and `/api/v1/tasks:invoke` to activate a durable TeamRun with one Lead and a non-empty declared roster. `TeamDriver` links bounded Lead turns, work attempts, and addressed continuations to MemberRuns. Lead commands create, accept, or request changes to Work; all Work must be accepted before the Lead can finish. TeamMessage provides durable deduplicated direct delivery and wake state. TeamRun reads remain owner-scoped, and the fixed Team is exposed through the Project-first same-origin Web BFF. AgentProject authoring accepts inline Agent, Environment, and Team specs; fills only approved constant/safe boilerplate defaults before canonicalization; and exposes explicit built-in Team Lead/member tool-profile refs. Equivalent short and fully explicit declarations converge on the same fingerprint and published artifacts. Invocation-time definitions remain prohibited. The managed Agent package contract does not claim arbitrary model execution or that referenced tools and skills are available merely because their references parse.
...
The Agent Teams v2 project view adds a fixed local/single-operator Project-first
