...
function Probe() {
  const query = useWorkDetail({workId: '11111111-1111-4111-8111-111111111111', preferCurrentDefinition: true, includeTrace: false});
...
it.each([
  ...
  [503, null, 'starting', 2],
  [403, null, 'error', 1],
] as const)('classifies status %s / %s without endless false starting', async (status, code, expected, calls) => {
  vi.useFakeTimers();
  vi.mocked(loadWorkDetail).mockRejectedValue(new ProductReadError('private backend detail', status, code));
