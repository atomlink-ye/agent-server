  ...
  return (
    <>
      <p className="work-shell-breadcrumb">
        ...
        <a href={workRootPath(originConversationId)}>{t('work.myWork')}</a> /{' '}
        {work.title}
      </p>
      <header className="work-detail-header">
        <div className="work-detail-header__copy">
          ...
          {!run ? (
            <p className="work-detail-surface-note">{t('work.startRunHint')}</p>
          ) : run.work_run.product_state === 'complete' ? (
            <p className="work-detail-surface-note">{t('work.completeHint')}</p>
          ) : run.work_run.product_state === 'problem' ? (
            <p className="work-detail-surface-note">{t('work.problemHint')}</p>
          ) : run.work_run.product_state === 'not_captured' ? (
            <p className="work-detail-surface-note">
              {t('work.notCapturedHint')}
            </p>
          ) : (
            <p className="work-detail-surface-note">{t('work.activeHint')}</p>
          )}
        </div>
        {actions ? (
