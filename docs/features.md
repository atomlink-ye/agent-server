...
| Agents and Teams                 | Baseline       | **Collaborative Team MVE plus compatibility paths:** managed Agent/Team YAML validation, owner-scoped registry/API, immutable published versions, published-only Task admission, durable TeamRun/MemberRun/WorkItem reads, independent member RuntimeSessions, preserved `sequential-mvp-v1`, and opt-in `dag-mve-v1` with two parallel leaves, durable join, and synthesizer                                                                                                                                                                                                                                           | Immutable Agent/Team versions and bounded graphs                                                      |
...
The Collaborative Team MVE uses the registry routes and `/api/v1/tasks:invoke` to create a durable TeamRun with one lead and a persisted roster. TeamRun, MemberRun, and WorkItem reads are owner-scoped; the lead prompt uses actual roster names and finalization requires exactly one completed, member-owned WorkItem per roster member, completed member Tasks, and succeeded latest Runs. Members use the team MCP tools through independent RuntimeSessions. Lead finalization uses a bounded prompt and a fresh task-scoped runtime execution/provider Agent while the lead member's canonical session remains the kickoff team_member session. `sequential-mvp-v1` remains the existing compatibility subset. The opt-in `dag-mve-v1` path is observed to materialize two parallel leaf child Tasks/Runs, wait at a root Run in `waiting_children`, durably join both successes, then materialize a synthesizer child and complete the root. The fixed self-learning Project/Team starter is implemented and its local Project Lab launches the approved three-member Team through the same-origin BFF. The managed Agent package contract does not claim arbitrary model execution or that referenced tools and skills are available merely because their references parse.
...
The self-learning Project Lab adds a fixed local/single-operator observation and
review surface. Its same-origin strict BFF launches the configured Project/Team,
projects only a bounded Team/report/activity/proposal/Memory aggregate, and
reconstructs that aggregate on refresh. Review is root-bound and uses the
canonical LearningProposal-to-Memory CAS path. The BFF keeps the server bearer
server-side; it exposes no runtime Memory write capability, provider/runtime IDs,
local paths, prompts, raw events, or raw upstream errors. Deployment beyond the
trusted local operator requires a new authentication Human Gate.
...
- Broader Web console behavior, cancel UI, old-session restart recovery, and production Web identity/security hardening;
