  ...
  implements CollaborationRepository
{
  ...
  public async createOpenWork(
    input: Parameters<CollaborationRepository['createOpenWork']>[0],
  ) {
    return this.transaction(async (client) => {
      await this.rejectDuplicateCommand(
        client,
        input.sourceRunId,
        input.commandHash,
      );
      const team = await this.lockTeam(
        client,
        input.teamRunId,
      ...
      await this.recordCommand(
        ...
        input.sourceRunId,
        input.commandHash,
        'collaboration_board_create_open',
        { work_item_id: id },
        now,
      );
  ...
  public async assignOpenWork(
    input: Parameters<CollaborationRepository['assignOpenWork']>[0],
  ) {
    return this.assignOrClaim({ ...input, claimant: false });
  }

  public async claimOpenWork(
    input: Parameters<CollaborationRepository['claimOpenWork']>[0],
  ) {
  ...
  public async blockCurrentAttempt(
    input: Parameters<CollaborationRepository['blockCurrentAttempt']>[0],
  ) {
    return this.transaction(async (client) => {
      ...
      // A blocker is a semantic Workboard state, not a fifth runtime-attempt
      // state. Mark this attempt completed with the blocker summary so the
      // existing durable Run kernel can terminalize it and wake the Lead.
      const attempt = await client.query<AttemptRow>(
        `UPDATE team_work_item_attempts
            SET status='completed',result_summary=$5,completed_at=now(),updated_at=now()
      ...
      await client.query(
        'UPDATE team_runs SET revision=revision+1, updated_at=now() WHERE id=$1',
        [input.teamRunId],
      );
      return {
        item: mapWork(work.rows[0]),
        attempt: mapAttempt(attempt.rows[0]),
      };
    });
  }

  public async resumeBlockedWork(
    input: Parameters<CollaborationRepository['resumeBlockedWork']>[0],
  ) {
    return this.transaction(async (client) => {
      await this.rejectDuplicateCommand(
        client,
      ...
      if (!current.rows?.[0])
        throw new TeamExecutionError('invalid_transition');
      await assertAvailableMember(
        client,
        input.teamRunId,
        input.assigneeMemberId,
        input.owner,
      );
      await assertMemberHasNoPendingWork(
        client,
        input.teamRunId,
        input.assigneeMemberId,
        team.root_task_id,
      ...
      await this.recordCommand(
        ...
        input.sourceRunId,
        input.commandHash,
        'collaboration_board_resume',
        { work_item_id: input.workItemId, attempt_id: attemptId },
        now,
      );
      return {
        item: mapWork(work.rows[0]),
  ...
  public async recordCheckpoint(
    input: Parameters<CollaborationRepository['recordCheckpoint']>[0],
  ) {
    return this.transaction(async (client) => {
      const team = await client.query<{ root_task_id: string }>(
      ...
      if (input.attemptId) {
        const attempt = await client.query<{ id: string }>(
          `SELECT id FROM team_work_item_attempts WHERE id=$1 AND work_item_id=$2 AND team_run_id=$3
            AND assignee_member_id=$4 AND status IN ('running','completed') AND ${ownerSql('', 5)} FOR SHARE`,
          [
            input.attemptId,
            input.workItemId,
            input.teamRunId,
  ...
  public async submitCurrentAttempt(
    input: Parameters<CollaborationRepository['submitCurrentAttempt']>[0],
  ) {
    return this.transaction(async (client) => {
      const team = await client.query<{ root_task_id: string }>(
        `SELECT root_task_id FROM team_runs WHERE id=$1 AND status='active' AND ${ownerSql('', 2)} FOR UPDATE`,
  ...
  private async assignOrClaim(input: {
    ...
    return this.transaction(async (client) => {
      ...
      if (!attempt.rows?.[0] || !updated.rows?.[0])
        throw new TeamExecutionError('stale_state');
      await insertWakeMessage(client, {
        teamRunId: input.teamRunId,
      ...
      await this.recordCommand(
        ...
        input.commandHash,
        input.claimant
          ? 'collaboration_board_claim'
          : 'collaboration_board_assign',
        { work_item_id: input.workItemId, attempt_id: attemptId },
        now,
      );
      return {
        item: mapWork(updated.rows[0]),
        attempt: mapAttempt(attempt.rows[0]),
      };
    });
  }
  ...
  private async rejectDuplicateCommand(
    client: Queryable,
    sourceRunId: string,
    commandHash: string,
  ) {
...
async function assertLiveSource(
  client: Queryable,
  input: {
    taskId: string;
    runId: string;
...
async function assertAvailableMember(
  client: Queryable,
  teamRunId: string,
  memberId: string,
  owner: OwnerScope,
...
async function assertMemberHasNoPendingWork(
  client: Queryable,
  teamRunId: string,
  memberId: string,
  rootTaskId: string,
...
async function insertWakeMessage(
  client: Queryable,
  input: {
    teamRunId: string;
