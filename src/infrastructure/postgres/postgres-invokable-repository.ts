...
/**
 * PostgreSQL compatibility repository for Team invokables only.
 *
 * Managed Agents are persisted and resolved exclusively through
 * PostgresAgentRegistry / AgentResolutionApi. Keeping the Team repository
 * separate prevents the historical invokable Agent projection from becoming a
 * second Agent aggregate or writer again.
 */
export class PostgresInvokableRepository implements InvokableRepository {
  ...
  public async importTeamVersionAtomically(input: {
    ...
    return this.withTransaction(async (tx) => {
      ...
      if (!row) throw new Error('team_registry_claim_missing');
  ...
  private async findTeamVersionBySql(
    ...
    const row = result.rows?.[0];
    return row ? mapTeamVersionRow(row) : null;
  }
}

function definitionValues(definition: TeamDefinition): readonly unknown[] {
  return [
    definition.id,
    definition.tenantId,
