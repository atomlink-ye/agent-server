...
for (const locale of ['en', 'zh-CN'] as const) {
  it.each(['loading', 'ready', 'error', 'unavailable', 'denied'] as const)(
    `${locale} list %s has a stable surface`,
    async (status) => {
  ...
  it.each([
    ...
    'permission',
    'unavailable',
  ] as const)(`${locale} detail %s explains its state`, async (state) => {
    await page.viewport(1440, 900);
    setLocale(locale);
    vi.mocked(useWorkDetail).mockReturnValue({
      status: state === 'loading' || state === 'starting' ? state : 'error',
      detail: null,
      error:
        state === 'missing'
          ? new WorkDetailRootNotFoundError()
          : state === 'permission'
            ? new ProductReadError('private detail', 403)
            : state === 'unavailable'
              ? new ProductReadError(
                  'private detail',
                  503,
                  'feature_unavailable',
                )
    ...
    try {
        ...
        root.render(
          <MemoryRouter>
            ...
            <main className="chat-panel work-main">
              <section className="work-main-content scroll-region">
      ...
      if (state === 'starting')
        expect(shell.textContent).toContain(t('work.detail.startingBody'));
    ...
    try {
      await act(async () => {
        root.render(
          <MemoryRouter>
            <div />
            <WorkPage selectedWorkId="invalid" />
          </MemoryRouter>,
        );
      });
